...
export function OrderTable({ orders, products, users = [], promoters = [] }: OrderTableProps) {
  ...
  const todayAmount = todayOrders.reduce((sum: number, o) => sum + o.totalAmount, 0)
...
function OrderRow({ order, products, promoters }: { order: Order, products: Product[], promoters: Promoter[] }) {
  ...
  const totalAmountWithExtensions = order.totalAmount + (order.extensions || []).reduce((acc: number, curr) => acc + curr.price, 0)
  const totalExtensionDays = (order.extensions || []).reduce((acc: number, curr) => acc + curr.days, 0)
        ...
        {(order.extensions || []).length > 0 && (
            <div className="mt-1">
                <Badge variant="secondary" className="text-xs">
                    + 续租 {(order.extensions || []).reduce((acc: number, curr) => acc + curr.days, 0)} 天
                </Badge>
            </div>
        )}
      </TableCell>
      ...
      <TableCell className="align-top">
        ...
        {(order.extensions || []).length > 0 && (
             <div className="text-xs text-red-400 mt-1">
                (+ 续租 ¥{(order.extensions || []).reduce((acc: number, curr) => acc + curr.price, 0)})
             </div>
        )}
