...
// App base path — injected by electron main process
let _basePath = process.cwd()
export function setAppBasePath(p: string) { _basePath = p }
export function _appBasePath() { return _basePath }
...
/** Synchronous access to external config (for use inside ensureContext) */
export function getExternalConfig(): (OnlineOrdersConfig & Record<string, unknown>) | null {
  return _externalConfig as (OnlineOrdersConfig & Record<string, unknown>) | null
}

/** Close the browser context so next sync restarts with fresh args (e.g. new window position) */
...
/** Move the browser window. x=-4800,y=-4800 hides it off-screen; x=100,y=100 brings it back. */
export async function moveWindow(x: number, y: number) {
  ...
  try {
    ...
    await cdp.detach()
    // Also bring the active page to front
    if (runtime.page && !runtime.page.isClosed()) {
      await runtime.page.bringToFront().catch(() => void 0)
    }
  } catch { /* ignore if browser not open */ }
}
...
async function ensureContext(headlessConfig: boolean = false): Promise<BrowserContext> {
  if (runtime.context) {
    try {
      ...
      if ((runtime as Record<string, unknown>)._isShared) {
  ...
  const userDataDir = path.join(_appBasePath(), ".playwright", "zanchen")
  ...
  // In production (when not on Windows dev machine), prefer headless
    ...
    // Read window position from injected config (_showBrowser flag set by main process)
    const showBrowser = (_externalConfig as unknown as Record<string, unknown>)?._showBrowser !== false
    const windowArgs = showBrowser ? [] : ['--window-position=-4800,-4800']
...
async function openZanchenOrderListByClicks(page: Page) {
  ...
  for (const sel of steps) {
    // Short timeout per step — if selector doesn't exist, skip rather than hang
    try {
      const loc = page.locator(sel).first()
      ...
      addLog(`[System] Nav step skipped (not found): ${sel}`)
      // Continue — maybe we're already past this step
    }
  }
  ...
  try {
    await page.waitForLoadState("domcontentloaded", { timeout: 10000 }).catch(() => void 0)
  } catch {
    void 0
  }
  return true
...
async function runZanchenSync(siteId: string) {
  ...
  // sync-tool: skip isEnabled check — user manually triggered sync
  ...
  if (orderUrl && (currentOrderUrl.includes(orderUrl) || (orderUrl.startsWith("http") && currentOrderUrl.startsWith(orderUrl)))) {
      ...
      // Verify we actually landed on the order list — check container visibility
      const container = getContainerSelector(site.selectors)
      let onOrderPage = false
      if (container) {
        try {
          await page.waitForSelector(container, { state: "visible", timeout: 2500 })
          onOrderPage = true
        } catch { /* not visible */ }
      } else if (orderUrl && page.url().includes(orderUrl.split('?')[0])) {
        onOrderPage = true
      }

      if (!onOrderPage && orderUrl) {
        addLog("[System] Click nav didn't reach order page, falling back to direct URL...")
        try {
          const origin = new URL(orderUrl).origin
          await page.goto(orderUrl, { waitUntil: "domcontentloaded", referer: `${origin}/` })
          await page.waitForTimeout(1500)
        } catch (e) {
          addLog(`[System] Direct URL fallback failed: ${e}`)
        }
      } else if (!onOrderPage) {
  ...
  const stopThreshold = site.stopThreshold ?? config?.stopThreshold ?? 20
